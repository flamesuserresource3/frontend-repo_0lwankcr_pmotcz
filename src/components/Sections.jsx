...
import { Reveal, TiltCard, MaskImage } from './AnimatedSections'
        ...
        <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {items.map((it, idx) => (
            <Reveal key={it.title} delay={0.05 * idx}>
              <TiltCard>
                <div className="rounded-2xl border border-black/[0.06] bg-white p-6 shadow-sm transition will-change-transform hover:-translate-y-1 hover:shadow-[0_0_24px_rgba(255,209,102,0.25)]">
    ...
    <section id="progetti" className="bg-[#FAFAFA]">
      <div className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2" role="group" aria-label="Filtri progetti">
            <Filter size={18} className="text-[#1A1A1A]/60" aria-hidden="true" />
            {filters.map((f, i) => (
              <Reveal key={f} delay={0.05 * i}>
                <button className="rounded-full border border-[#1A1A1A]/10 bg-white px-3 py-1 text-xs text-[#1A1A1A]/80 hover:text-[#1A1A1A] focus:outline-none focus:ring-2 focus:ring-[#FFD166]">
                  {f}
                </button>
              </Reveal>
            ))}
        ...
        <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {projects.map((p, idx) => (
            <Reveal key={p.title} delay={0.06 * idx}>
              <TiltCard>
                <article className="group overflow-hidden rounded-2xl border border-black/[0.06] bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-[0_0_24px_rgba(255,209,102,0.25)]">
                  <MaskImage src={p.img} alt={p.title} className="aspect-video" />
                  <div className="p-5">
                    ...
                    <h3 className="text-lg font-semibold text-[#1A1A1A] group-hover:text-[#E63946] transition-colors">{p.title}</h3>
          ...
          {values.map((v, i) => (
            <Reveal key={v.title} delay={0.06 * i}>
              <motion.div
                ...
                className="rounded-2xl border border-black/[0.06] bg-white p-6 text-center shadow-sm"
              >
                <motion.div
                  className="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-[#1A1A1A] text-white"
                  animate={{ scale: [1, 1.06, 1], boxShadow: ['0 0 0 rgba(255,209,102,0)', '0 0 24px rgba(255,209,102,0.6)', '0 0 0 rgba(255,209,102,0)'] }}
        ...
        <div className="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {testimonials.map((t, i) => (
            <Reveal key={t.name} delay={0.04 * i}>
              <blockquote className="rounded-2xl border border-black/[0.06] bg-[#FAFAFA] p-6 text-[#1A1A1A] shadow-inner">
  ...
  return (
    <section id="contatti" className="relative overflow-hidden bg-[#1A1A1A]">
      <div className="absolute inset-0 pointer-events-none" aria-hidden="true">
        <motion.div
          className="absolute -top-10 left-1/2 h-64 w-64 -translate-x-1/2 rounded-full bg-[#FFD166]/30"
          animate={{ scale: [1, 1.08, 1], opacity: [0.6, 1, 0.6] }}
          transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut' }}
        />
        <motion.div
          className="absolute bottom-0 right-10 h-40 w-40 rounded-full bg-[#E63946]/20"
        ...
        <div className="grid items-center gap-10 lg:grid-cols-2">
          <div>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <motion.a
                ...
                target="_blank"
                rel="noreferrer"
                className="rounded-full border border-white/30 bg-white/10 px-5 py-2 text-sm font-medium text-white backdrop-blur focus:outline-none focus:ring-2 focus:ring-[#FFD166]"
              ...
              <motion.a
                href="#"
                className="rounded-full bg-[#E63946] px-5 py-2 text-sm font-medium text-white shadow-[0_0_30px_rgba(230,57,70,0.6)] focus:outline-none focus:ring-2 focus:ring-[#FFD166]"
                whileHover={{
                  boxShadow: [
          ...
          <Reveal>
            <form className="rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur">
              ...
              <motion.button type="submit" className="mt-5 w-full rounded-full bg-[#E63946] px-6 py-3 text-sm font-semibold text-white shadow-[0_0_30px_rgba(230,57,70,0.6)] focus:outline-none focus:ring-2 focus:ring-[#FFD166]"
